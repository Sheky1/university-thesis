...
import { updateAgency } from "../../store/actions/index";
  ...
  onSubmit = async () => {
    try {
      if (this._isMounted) {
        ...
        const { name, address, username, email, agency_id, password, files } =
          this.state;
        ...
        if (password !== "") updatedAgency = { ...updatedAgency, password };
        if (files.length > 0)
